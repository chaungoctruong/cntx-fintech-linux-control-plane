WITH candidate_deployments AS (
    ...
    WHERE d.status = ANY(%s)
      ...
      AND (
          d.status = 'running'
          OR COALESCE(d.updated_at, d.created_at) < (NOW() - (%s * INTERVAL '1 second'))
      )
      AND NOT EXISTS (
          SELECT 1
          FROM execution_commands start_cmd
          WHERE start_cmd.deployment_id = d.id
            AND start_cmd.command_type = 'START_BOT'
            AND start_cmd.delivery_status IN ('pending', 'queued', 'dispatched')
            AND start_cmd.updated_at >= (NOW() - (%s * INTERVAL '1 second'))
      )
      AND (
          (
              ...
              AND LOWER(COALESCE(s.metadata_json->>'account_id', '')) IN ('', 'null', 'none', '0')
              ...
              AND LOWER(COALESCE(s.metadata_json->>'active_deployment_id', '')) IN ('', 'null', 'none', '0')
          )
          OR (
              (
                  CASE
              ...
              AND NOT EXISTS (
                  SELECT 1
                  FROM runner_slots active_slot
                  WHERE active_slot.runner_id = d.runner_id
                    AND active_slot.status = 'allocated'
              )
          )
      )
...
updated_deployments AS (
    ...
    SET status = 'stopped',
        ...
        is_active = FALSE,
        health_status = 'runner_reported_zero_runtime',
        stopped_at = COALESCE(d.stopped_at, NOW()),
        last_error = COALESCE(NULLIF(d.last_error, ''), 'runner_reported_zero_runtime'),
        updated_at = NOW()
    ...
    SET current_account_id = NULL,
        ...
        metadata_json = jsonb_strip_nulls(
            (
                ...
                'runtime_reconcile_reason', 'runner_reported_zero_runtime'
            )
        ),
        updated_at = NOW()
...
failed_pending_start_commands AS (
    UPDATE execution_commands c
    SET delivery_status = 'failed',
        last_error = COALESCE(NULLIF(c.last_error, ''), 'runner_reported_zero_runtime_after_start'),
        updated_at = NOW()
    FROM updated_deployments d
...
acknowledged_pending_stop_commands AS (
    UPDATE execution_commands c
    SET delivery_status = 'acknowledged',
        acknowledged_at = COALESCE(c.acknowledged_at, NOW()),
        last_error = COALESCE(NULLIF(c.last_error, ''), 'runner_reported_zero_runtime_after_stop'),
        updated_at = NOW()
    FROM updated_deployments d
    WHERE c.deployment_id = d.id
...
SELECT
    (SELECT COUNT(*) FROM updated_deployments) AS reconciled_active_zero_runtime_deployments,
    (SELECT COUNT(*) FROM released_slots) AS reconciled_active_zero_runtime_slots,
    (SELECT COUNT(*) FROM failed_pending_start_commands) AS failed_zero_runtime_start_commands,
    (SELECT COUNT(*) FROM acknowledged_pending_stop_commands) AS acknowledged_zero_runtime_stop_commands
