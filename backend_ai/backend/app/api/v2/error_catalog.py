...
_ENTRIES: tuple[ErrorEntry, ...] = (
    ...
    ErrorEntry(
        public_code="bot_runtime_lane_required",
        http_status=status.HTTP_400_BAD_REQUEST,
        message_vi="Bot này thuộc lane runtime riêng. Vui lòng chọn đúng loại bot trong giao diện mới.",
        message_en="This bot belongs to a dedicated runtime lane. Please select it from the matching bot lane in the updated interface.",
        action="fix_input",
        retryable=False,
        group="bot",
    ),
    ErrorEntry(
        public_code="bot_runtime_lane_mismatch",
        http_status=status.HTTP_400_BAD_REQUEST,
        message_vi="Loại bot bạn chọn không khớp với lane runtime của bot. Vui lòng tải lại danh sách bot và chọn lại.",
        message_en="The selected bot type does not match the bot runtime lane. Reload the bot catalog and choose again.",
        action="reload",
        retryable=False,
        group="bot",
    ),
    ErrorEntry(
        public_code="direct_ea_runtime_not_enabled",
        http_status=status.HTTP_403_FORBIDDEN,
        message_vi="Lane bot EA MT5 chưa được bật cho người dùng. Vui lòng chờ backend bật Direct-EA runtime contract.",
        message_en="The MT5 EA bot lane is not enabled for users yet. Wait until the Direct-EA runtime contract is enabled.",
        action="contact_support",
        retryable=False,
        group="bot",
    ),
    ...
    ErrorEntry(
        public_code="mt5_login_failed",
        http_status=status.HTTP_409_CONFLICT,
        message_vi="Thông tin tài khoản MT5 chưa đúng. Vui lòng kiểm tra login, mật khẩu hoặc server.",
        message_en="MT5 login failed. Check the server, account number, and password, then reconnect.",
        action="retry_login",
        retryable=False,
        group="login_slot",
    ),
    ...
    # ---------------------------------------------------------------
    ErrorEntry(
        public_code="tradingview_contract_version_required",
        http_status=status.HTTP_400_BAD_REQUEST,
        message_vi="Webhook TradingView thiếu contract_version nên backend từ chối để tránh chạy alert cũ.",
        message_en="TradingView webhook is missing contract_version, so the backend rejected it to avoid executing an old alert contract.",
        action="fix_input",
        retryable=False,
        group="tradingview",
    ),
    ErrorEntry(
        public_code="tradingview_contract_version_unsupported",
        http_status=status.HTTP_400_BAD_REQUEST,
        message_vi="contract_version của webhook TradingView không còn được hỗ trợ. Vui lòng cập nhật Pine/alert mới.",
        message_en="TradingView webhook contract_version is no longer supported. Please update the Pine script/alert payload.",
        action="fix_input",
        retryable=False,
        group="tradingview",
    ),
    ErrorEntry(
        public_code="tradingview_alert_time_required",
        http_status=status.HTTP_400_BAD_REQUEST,
        message_vi="Webhook TradingView thiếu alert_time_ms nên backend không thể kiểm tra độ mới của alert.",
        message_en="TradingView webhook is missing alert_time_ms, so the backend cannot verify alert freshness.",
        action="fix_input",
        retryable=False,
        group="tradingview",
    ),
    ErrorEntry(
        public_code="tradingview_alert_stale",
        http_status=status.HTTP_400_BAD_REQUEST,
        message_vi="Alert TradingView đã quá cũ nên backend từ chối để tránh vào lệnh trễ.",
        message_en="TradingView alert is too old, so the backend rejected it to avoid delayed execution.",
        action="retry",
        retryable=False,
        group="tradingview",
    ),
    ErrorEntry(
        public_code="tradingview_alert_clock_skew",
        http_status=status.HTTP_400_BAD_REQUEST,
        message_vi="Thời gian alert TradingView lệch quá xa so với backend. Vui lòng kiểm tra lại payload.",
        message_en="TradingView alert time is too far ahead of backend time. Please check the payload.",
        action="fix_input",
        retryable=False,
        group="tradingview",
    ),
    ErrorEntry(
        public_code="tradingview_volume_exceeds_limit",
        http_status=status.HTTP_400_BAD_REQUEST,
        message_vi="Khối lượng lệnh TradingView vượt giới hạn an toàn của backend.",
        message_en="TradingView order volume exceeds the backend safety limit.",
        action="fix_input",
        retryable=False,
        group="tradingview",
    ),
    ErrorEntry(
        public_code="tradingview_entry_price_required",
        http_status=status.HTTP_400_BAD_REQUEST,
        message_vi="Webhook TradingView thiếu entry_price nên backend không thể tính SL/TP an toàn cho runner.",
        message_en="TradingView webhook is missing entry_price, so the backend cannot safely calculate runner SL/TP distances.",
        action="fix_input",
        retryable=False,
        group="tradingview",
    ),
    ErrorEntry(
        public_code="tradingview_dca_price_required",
        http_status=status.HTTP_400_BAD_REQUEST,
        message_vi="Webhook TradingView bật DCA limit nhưng thiếu dca_price/dca_limit_price.",
        message_en="TradingView webhook requested DCA limit but is missing dca_price/dca_limit_price.",
        action="fix_input",
        retryable=False,
        group="tradingview",
    ),
    ErrorEntry(
        public_code="tradingview_entry_type_invalid",
        http_status=status.HTTP_400_BAD_REQUEST,
        message_vi="Kiểu vào lệnh TradingView không hợp lệ. Backend chỉ hỗ trợ market hoặc limit.",
        message_en="TradingView entry type is invalid. Backend only supports market or limit.",
        action="fix_input",
        retryable=False,
        group="tradingview",
    ),
    ErrorEntry(
        public_code="tradingview_dca_limit_runner_unsupported",
        http_status=status.HTTP_400_BAD_REQUEST,
        message_vi="Runner hiện tại chưa khai báo hỗ trợ lệnh pending/limit, nên backend bỏ qua DCA limit để tránh vào nhầm market.",
        message_en="The current runner has not advertised pending/limit order support, so the backend skipped the DCA limit to avoid accidental market execution.",
        action="contact_support",
        retryable=False,
        group="tradingview",
    ),
    ErrorEntry(
        public_code="tradingview_strategy_mismatch",
        http_status=status.HTTP_400_BAD_REQUEST,
        message_vi="Strategy của webhook TradingView không khớp với subscription nên backend bỏ qua subscriber này.",
        message_en="TradingView webhook strategy does not match the subscription, so the backend skipped this subscriber.",
        action="fix_input",
        retryable=False,
        group="tradingview",
    ),
    ErrorEntry(
        public_code="tradingview_runtime_lane_mismatch",
        http_status=status.HTTP_400_BAD_REQUEST,
        message_vi="Webhook TradingView chỉ được gửi lệnh vào bot thuộc lane backend_webhook_signal.",
        message_en="TradingView webhooks may only dispatch orders to bots in the backend_webhook_signal lane.",
        action="fix_input",
        retryable=False,
        group="tradingview",
    ),
    ErrorEntry(
        public_code="tradingview_price_levels_invalid",
        http_status=status.HTTP_400_BAD_REQUEST,
        message_vi="SL/TP TradingView không đúng chiều so với entry_price. Vui lòng kiểm tra lại alert.",
        message_en="TradingView SL/TP levels are not valid for the side and entry_price. Please check the alert.",
        action="fix_input",
        retryable=False,
        group="tradingview",
    ),
    ErrorEntry(
        public_code="tradingview_price_distance_below_minimum",
        http_status=status.HTTP_400_BAD_REQUEST,
        message_vi="Khoảng cách SL/TP TradingView quá nhỏ so với ngưỡng an toàn của backend.",
        message_en="TradingView SL/TP distance is below the backend safety minimum.",
        action="fix_input",
        retryable=False,
        group="tradingview",
    ),
    ErrorEntry(
        public_code="tradingview_price_distance_exceeds_limit",
        http_status=status.HTTP_400_BAD_REQUEST,
        message_vi="Khoảng cách SL/TP TradingView vượt giới hạn an toàn của backend.",
        message_en="TradingView SL/TP distance exceeds the backend safety limit.",
        action="fix_input",
        retryable=False,
        group="tradingview",
    ),
    ErrorEntry(
        ...
        http_status=status.HTTP_503_SERVICE_UNAVAILABLE,
        message_vi="Hệ thống hàng đợi tạm thời không sẵn sàng. Vui lòng thử lại sau vài giây.",
        message_en="The queue service is temporarily unavailable. Please retry shortly.",
        action="retry",
        retryable=True,
        group="system",
    ),
    ErrorEntry(
        ...
        http_status=status.HTTP_503_SERVICE_UNAVAILABLE,
        message_vi="Cơ sở dữ liệu tạm thời không sẵn sàng. Vui lòng thử lại sau vài giây.",
        message_en="The database is temporarily unavailable. Please retry shortly.",
        action="retry",
        retryable=True,
        group="system",
    ),
